...
async function run() {
  ...
  for (const lp of landingPages) {
    if (lp.logo) {
      ...
      if (lp.logo.startsWith("data:image")) {
        console.log(`-> Logo is a base64 data URL. Cleaning it...`);
  ...
  // Also let's check if there are other base64 images in User or Doctor profileImages
  console.log("\nChecking Doctor profile images...");
