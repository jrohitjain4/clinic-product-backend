...
const createService = async (req, res) => {
    try {
        ...
        if (!clinicId)
            return res.status(403).json({ message: "No clinic associated" });
        const { serviceName, serviceCode, serviceType, description, gallery, minSessions, maxSessions, sessionGap, scheduleType, customDates, departmentId, specializationId, price, duration, status } = req.body;
        if (!serviceName) {
