...
const modeFromAppointmentType = (appointmentType) => {
    if (!appointmentType || appointmentType === "Select")
        return "In-person";
    if (appointmentType.toLowerCase().includes("online"))
        return "Online";
    return "In-person";
};
...
const getAppointments = async (req, res) => {
    try {
        ...
        if (req.user?.role === "DOCTOR" && req.user?.email) {
            const loggedInDoctor = await prisma_1.default.doctor.findFirst({
        ...
        const appointments = await prisma_1.default.appointment.findMany({
            ...
            orderBy: sort === "oldest" ? { scheduledAt: "asc" } : { scheduledAt: "desc" },
        });
        ...
        for (const app of enriched) {
            ...
            if (!queueCache[cacheKey]) {
                ...
                queueCache[cacheKey] = await prisma_1.default.appointment.findMany({
                    where: {
...
exports.getAppointmentsCalendar = getAppointmentsCalendar;
...
const createAppointment = async (req, res) => {
    try {
        ...
        const patient = await prisma_1.default.patient.findFirst({ where: { id: patientId, clinicId } });
        if (!patient)
            return res.status(400).json({ message: "Invalid patient" });
        const doctor = await prisma_1.default.doctor.findFirst({ where: { id: doctorId, clinicId } });
        if (!doctor)
...
const updateAppointment = async (req, res) => {
    try {
        ...
        if (targetDoctorId) {
            doctor = await prisma_1.default.doctor.findFirst({
