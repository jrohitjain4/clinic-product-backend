...
const createPayroll = async (req, res) => {
    try {
        ...
        if (!clinicId)
            return res.status(403).json({ message: "No clinic associated" });
        const { staffId, doctorId, netSalary, basicSalary, da, hra, conveyance, medicalAllowance, otherEarnings, tds, esi, pf, profTax, labourWelfare, otherDeductions, status, salaryDate } = req.body;
        ...
        const existingNext = await prisma_1.default.payroll.findFirst({
            where: {
                clinicId,
                staffId: staffId || null,
                doctorId: doctorId || null,
        ...
        if (!existingNext) {
            await prisma_1.default.payroll.create({
                data: {
                    staffId: staffId || null,
                    doctorId: doctorId || null,
                    netSalary: Number(netSalary),
